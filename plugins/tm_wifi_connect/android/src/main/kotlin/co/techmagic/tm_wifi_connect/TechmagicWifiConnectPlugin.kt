...
class TechmagicWifiConnectPlugin() : FlutterPlugin, MethodCallHandler {
  ...
  override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    ...
    channel = MethodChannel(flutterPluginBinding.binaryMessenger, "tm_wifi_connect")
  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
    when (call.method) {
      ...
      "activateWifi" -> {
        setWifiState(true, result)
        return
      }
      "deactivateWifi" -> {
        setWifiState(false, result)
        return
      }
      "disconnect" -> {
        when {
  ...
  // /
  // / On API < 29 toggles it directly via WifiManager. On API >= 29 (where the
  // / toggle is a no-op) opens the system WiFi settings panel. Either way it
  // / then polls isWifiEnabled every 500ms for up to 30s, replying true once the
  // / target state is reached or false on timeout.
  @Suppress("DEPRECATION")
  private fun setWifiState(target: Boolean, @NonNull result: Result) {
    if (wifiManager.isWifiEnabled == target) {
      result.success(true)
      return
    }
    ...
    pollWifiState(target, result)
  }

  // / Polls isWifiEnabled twice per second for 30s. Single-reply guarded.
  private fun pollWifiState(target: Boolean, @NonNull result: Result) {
    val handler = Handler(Looper.getMainLooper())
    var ticks = 0
    val maxTicks = 60 // 60 * 500ms = 30s
    var replied = false
