    ...
    NEHotspotConfigurationManager.shared.apply(hotspotConfig) { (error) in
      if let error = error as NSError? {
        ...
        case NEHotspotConfigurationError.alreadyAssociated.rawValue:
            NSLog("[flutter_wifi_connect] apply: alreadyAssociated -> success")
            result(true)
        case NEHotspotConfigurationError.userDenied.rawValue:
            NSLog("[flutter_wifi_connect] apply: userDenied")
            result(false)
        default:
            NSLog("[flutter_wifi_connect] apply failed: code=\(error.code) \(error.localizedDescription)")
            result(false)
        }
        return
      }
