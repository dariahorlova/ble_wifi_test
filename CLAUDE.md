...
- **`BleWifiCubit`** (`lib/features/connect/cubit/`) — single source of truth. State is freezed (`ble_wifi_state.dart`): `status` enum, `hintText`, and `devices` (a `List<ReaderDevice>`). There is **no** index/flag for the active device — `state.connectedDevice` is derived from the device whose `connection != none`. UI hints are transient — `_temporaryHint` emits a message then clears it after a delay (only if still unchanged). The cubit subscribes to `_bleRepository.connectionStream` and, on an unexpected drop, resets that device's `connection` to `none`; a `_wifiHandoffInProgress` flag suppresses this during `makeMagic`. Key methods: `searchBLE`, `connectToDevice(ReaderDevice)`, `getDeviceConfig(ReaderDevice)`, `updateDateTimeOnBle`, `disconnectBLE`, `makeMagic`, `launchNFC`.
- **`ReaderDevice`** (`models/reader_device.dart`, freezed) — the unit the UI renders: wraps the `BluetoothDevice` handle, a `ReaderConnection` enum (`none`/`ble`/`wifi`, in `enums/`), and the parsed `DeviceConfig?`. Getters: `id` (BLE remoteId), `name` (trimmed advName), `readerId` (name without `blePrefix`), `isConnected`/`isBleConnected`/`isWifiConnected`. Holds the identity constant `blePrefix = 'IB_'`.
- **`BleRepository`** (`repository/ble_repository.dart`) — all BLE protocol constants and command logic. Service/characteristic UUIDs (`serviceUuid`, `commandCharacteristicId`, `listenCharacteristicId`, `statusCharacteristicId`) and command byte arrays (all `[9, x]`): wifi-enable `[9,4]`, get-content `[9,3]`, factory-reset `[9,11]`, set-datetime `[9,12]`, update-config `[9,14]`, reboot `[9,15]`. Every command runs through `_runBleTask` (logging + try/catch + fallback). Exposes `connectionStream` (forwarded from `BLEService`) so the cubit can reconcile unexpected disconnects.
...
2. join the device's WiFi (SSID = `INFINIBOOK_` + `connectedDevice.readerId`, password `INFINIBOOK`); disconnect BLE and flip the device's `connection` to `wifi`
